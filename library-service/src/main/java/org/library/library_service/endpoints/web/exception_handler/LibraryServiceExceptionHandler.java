package org.library.library_service.endpoints.web.exception_handler;
...
public class LibraryServiceExceptionHandler {
    ...
    @ExceptionHandler(IllegalDatePeriodException.class)
    public ResponseEntity<ErrorResponse> handleDateError(IllegalDatePeriodException exception){
        ErrorResponse response = new ErrorResponse(ErrorType.ERROR, exception.getMessage());
        return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
    }
